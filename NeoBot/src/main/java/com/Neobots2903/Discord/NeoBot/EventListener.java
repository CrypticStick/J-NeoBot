...
public class EventListener extends ListenerAdapter {
	...
	public void logMessage(Message message, boolean edited)
    {
        ...
        try {
        	...
        	if (edited) {
            	...
            	eb.setDescription(String.format("Edited %s", message.getEditedTime().atZoneSameInstant(ZoneId.of("America/Los_Angeles")).format(outputFormat)));
        	} else {
        		eb.setColor(Color.red);
            	eb.setDescription(String.format("Sent %s", message.getCreationTime().atZoneSameInstant(ZoneId.of("America/Los_Angeles")).format(outputFormat)));
        	}
	...
	public void onReady(ReadyEvent e) {
		System.out.println(String.format("%s invite link: https://discordapp.com/oauth2/authorize?client_id=495375587339403264&scope=bot&permissions=8 \n",e.getJDA().getSelfUser().getName(),e.getJDA().getSelfUser()));
	...
	public void onGuildMemberJoin(GuildMemberJoinEvent e) {
		...
		c.queue((channel) -> channel
				.sendMessage(String.format(
						"Welcome %s! Make sure to read the rules and have fun!",
						NeoBot.jda.getSelfUser().getName()))
				.queue());
	}
	...
	public void onGuildMessageReactionAdd(GuildMessageReactionAddEvent e) {
		if (e.getUser().getId().equals(NeoBot.jda.getSelfUser().getId())) return;
		//more code here :)
	}
